...
{
    #pragma omp parallel for
    for (int i = 0; i < ssspTrees.size(); ++i) // Loop over the size of sources (or ssspTrees)
    {
...
void MOSP_Update::createCombinedGraph()
{
    ...
    #pragma omp parallel for
    for (int i = 0; i < ssspTrees.size(); ++i) 
    {
        const auto& graph = ssspTrees[i].getGraph(); // Get the adjacency list from SOSP_Update
        #pragma omp parallel for
        for (int u = 0; u < numVertices; ++u) 
        {
            #pragma omp critical
    ...
    for (int u = 0; u < numVertices; ++u) 
    {
        for (auto& edge : combinedGraph[u]) 
        {
            ...
            for (int i = 0; i < ssspTrees.size(); ++i) 
            {
                const auto& graph = ssspTrees[i].getGraph();
...
void MOSP_Update::findSOSPInCombinedGraph() 
{
    ...
    while (!pq.empty()) 
    {
        ...
        #pragma omp parallel for
        for (const auto& [v, weight] : combinedGraph[u]) 
        {
            #pragma omp critical
            {
